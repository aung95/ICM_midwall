...
" Suppl table S3"
```{r Tab6-descr-all-LGE_midwall_location}
...
" Suppl table S34
```{r Tab7-descr-all-LGE_midwall_isch_add}
df <- df_all %>% filter(CMR_LGE_midwall_presence == "B_Presence_of_midwall_LGE") %>% droplevels()
...
```{r Fig6-mid-KM-LGE_midwall_location}
df <- df_all %>% 
  filter(
    CMR_LGE_midwall_presence == "B_Presence_of_midwall_LGE"
    ) %>% 
...
```{r Fig7-mid-KM-LGE_midwall_extent}
df <- df_all %>% 
  filter(
    CMR_LGE_midwall_presence == "B_Presence_of_midwall_LGE"
    ) %>% 
...
```{r Fig8-mid-KM-LGE_midwall_with_ischemic}
df <- df_all %>% 
  filter(
    CMR_LGE_midwall_presence == "B_Presence_of_midwall_LGE"
    ) %>% 
